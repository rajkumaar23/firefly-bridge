...
3. A matching transaction is compared against that vendor's orders: same absolute amount (to the cent) and a compatible date. By default the charge must fall within `±date_window_days` of the order's `date`; when an order carries an [`effective_date`](#orders) (vendors that bill at shipment rather than order placement), the charge instead just has to fall in `[min(date, effective_date) − 1 day, max(date, effective_date) + date_window_days]`.
...
| `effective_date` | no | The date the charge is actually anchored to when the vendor bills at a moment other than order placement (e.g. Amazon bills at shipment/delivery, not when the order was placed). Parsed with `date_format` plus common layouts, and may omit the year ("August 21") — the year is inferred from `date` (nearest candidate year within 45 days, so December orders delivered in January wrap around correctly). Full dates are sanity-checked the same way: one more than 45 days from `date` is dropped rather than trusted. When present, a charge matches if it falls in `[min(date, effective_date) − 1 day, max(date, effective_date) + date_window_days]` instead of the symmetric `±date_window_days` around `date`. Omit it (or leave it empty) and matching falls back to the plain symmetric window. |
